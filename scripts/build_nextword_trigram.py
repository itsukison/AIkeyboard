#!/usr/bin/env python3
"""Build the next-word *trigram* prior from NWC2010 word 3-grams.

Sibling of `build_nextword_prior.py` (the bigram builder). Same output binary
format (magic "NWP1") so `NextWordPrior` reads it unchanged — but keyed on the
*pair* of preceding morphemes (t1, t2) joined by U+001F, mapping to the likely
third morpheme t3. At runtime `NextWordPrior.suggestions(after:_:)` builds the
same composite key and binary-searches it; `KanaKanjiAdapter` backs off to the
bigram table on a miss.

Input: stdin = decompressed NWC2010 word 3-gram stream ("t1 t2 t3\tfreq" per
       line, sorted lexicographically by "t1 t2 t3" so each (t1, t2) prefix
       forms a contiguous block).
Output: a compact binary file (see FORMAT) keyed by "t1<U+001F>t2".

FORMAT (little-endian) — identical to the bigram table:
  magic   : 4 bytes  b"NWP1"
  keyCount: uint32
  index   : keyCount * 12 bytes, each: keyOff:uint32 keyLen:uint16 valOff:uint32 valLen:uint16
            (offsets are absolute file offsets; keys sorted by UTF-8 bytes)
  keysBlob: concatenated key UTF-8 ("t1<U+001F>t2")
  valsBlob: per key, repeated entries: nextLen:uint8, next UTF-8, weight:uint8
"""
import sys, heapq, struct, math, io

SEP = "\x1f"        # must match NextWordPrior.keySeparator
TOP_K = 6           # next-words kept per (t1, t2) pair
MAX_KEYS = 200_000  # morpheme pairs kept (top by total frequency); tune vs size
BOUNDARY = {"<S>", "</S>", "<s>", "</s>"}
ALLOWED_PUNCT = {"。", "、", "！", "？", "」", "』", "）", "…", "ー"}
...
def main():
    ...
    for line in inp:
        ...
        if not t1 or not t2 or not t3:
            continue
        # Both key morphemes must be Japanese for the pair to be a useful key.
        if not has_jp(t1) or not has_jp(t2):
            continue
        if not keep_next(t3):
            continue
        try:
