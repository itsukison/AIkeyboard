...
public enum PromptOrigin: String, Codable, Sendable {
    /// One of the four seeded built-ins, unmodified or edited.
    case builtin
    /// Generated by the onboarding button builder from the user's selections.
    case onboardingBuilder = "onboarding_builder"
    /// A fixed preset the onboarding flow seeded without generating anything —
    /// the fallback when generation fails. Kept distinct from
    /// `onboardingBuilder` because a preset is not an authored button and the
    /// two predict retention very differently.
    case onboardingPreset = "onboarding_preset"
    /// Written by the user in the Prompts screen after onboarding.
    case userAuthored = "user_authored"
}

/// A worked example for a built button: the text the practice page pre-fills and
/// the three candidates the keyboard replays when the button is tapped.
///
/// Keyed by the button's id, not its title. The review page exists so the user
/// can rename the button they just made, and a title key silently detached the
/// example the moment they did.
public struct OnboardingGeneratedPractice: Codable, Equatable, Sendable {
    public let buttonId: String
    public let input: String
    public let outputs: [String]

    public init(buttonId: String, input: String, outputs: [String]) {
...
public struct UserPrompt: Codable, Equatable, Identifiable, Sendable {
    ...
    /// Rows written before `origin` existed — in the App Group cache and in every
    /// server row predating the column — decode without it. Infer rather than
